...
export class ComicChapterPageComponent implements OnInit {
    ...
    nextChapter() {
        ...
        this.router.navigate([`../../chapter/${index}`], { relativeTo: this.activeRoute }).then(() => window.location.reload())
    }
    ...
    navigateToChapterForSelect(event: any) {
        this.router.navigate([`../../chapter/${event.target.value}`], { relativeTo: this.activeRoute }).then(() => window.location.reload())
    }
